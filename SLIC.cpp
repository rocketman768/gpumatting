	...
	const double&				M)
{
	...
	for( int itr = 0; itr < 10; itr++ )
	{
		...
		{
			y1 = SLIC_MAX(0,			kseedsy[n]-offset);
			y2 = SLIC_MIN(m_height,	kseedsy[n]+offset);
			x1 = SLIC_MAX(0,			kseedsx[n]-offset);
			x2 = SLIC_MIN(m_width,	kseedsx[n]+offset);
	...
	const double&				m)
{
	...
	for( int itr = 0; itr < 5; itr++ )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
			...
			x1 = SLIC_MAX(0,			kseedsx[n]-offset);
			x2 = SLIC_MIN(m_width,	kseedsx[n]+offset);
			z1 = SLIC_MAX(0,			kseedsz[n]-offset);
			z2 = SLIC_MIN(m_depth,	kseedsz[n]+offset);
	...
	const std::string&				path) 
{
	...
	splitpath(filename.c_str(), fname, extn);
	std::string temp = fname;

	std::ofstream outfile;
	std::string finalpath = path + temp + std::string(".dat");
...
void SLIC::splitpath( const char* path, char* file, char* extension )
{
   int i, size;
   ...
   strncpy( extension, &path[i], size-i+1 );
   extension[size-i+1] = 0x00;
   strncpy( file, path, i );
   file[i] = 0x00;
}
